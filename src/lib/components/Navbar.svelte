...
<nav class="fixed top-0 left-0 w-full z-50 transition-all duration-300 {isScrolled ? 'bg-black/80 backdrop-blur-md border-b border-white/10' : 'bg-transparent'} text-white">
	<div class="max-w-[1400px] mx-auto px-6 h-20 flex items-center justify-between">
		<!-- Logo -->
		<a href="/" class="text-2xl font-serif tracking-tight font-medium">Greyhaven</a>
		...
		<div class="hidden lg:flex items-center gap-6">
			<a href="#contact" class="bg-[#3b82f6] hover:bg-blue-600 text-white px-5 py-2.5 rounded-full text-sm font-medium transition-colors">
	...
	{#if isMenuOpen}
		<div class="lg:hidden bg-black absolute top-20 left-0 w-full h-[calc(100vh-80px)] p-6 flex flex-col gap-6 overflow-y-auto border-t border-white/10">
			...
			<a href="#security" class="text-lg font-medium text-gray-300 hover:text-white">Security</a>
			...
			<a href="#contact" class="bg-[#3b82f6] text-white px-5 py-3 rounded-full text-center font-medium">
				Start a conversation
			</a>
		</div>
	{/if}
